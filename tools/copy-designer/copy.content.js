(function () {
  ...
  function markHoverables() {
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode(node) {
          ...
          if (
            !/(P|H1|H2|H3|H4|H5|H6|SPAN|LI|A|BUTTON|LABEL|DIV|SMALL|EM|STRONG)/.test(
              p.tagName
            )
          )
    ...
    while ((n = walker.nextNode())) {
      ...
      if (el && !seen.has(el)) {
        el.classList.add("__tdt_hoverable");
        seen.add(el);
      }
    }
    cleanupFns.push(() => {
      try {
        document
          .querySelectorAll(".__tdt_hoverable")
          .forEach((el) => el.classList.remove("__tdt_hoverable"));
      } catch (e) {}
    });
  }
    ...
    const esc = (e) => {
      if (e.key === "Escape") {
        const cur = document.querySelector(
          '.__tdt_editing[contenteditable="true"]'
        );
        if (cur) cur.blur();
      }
    };
  ...
  function edit(el) {
    try {
      ...
      const onBlur = () => {
        ...
        el.classList.remove("__tdt_editing");
  ...
  function exportDocx() {
    ...
    a.href = url;
    a.download =
      (document.title ? document.title.replace(/[^\w\-]+/g, "_") : "page") +
      "_copy_deck.docx";
  ...
  function teardown() {
    ...
    cleanupFns = [];
    try {
      document.querySelectorAll(".__tdt_editing").forEach((el) => {
        el.contentEditable = "false";
        el.classList.remove("__tdt_editing");
      });
    } catch (e) {}
