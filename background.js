...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'INJECT_TOOL') {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.tabs.sendMessage(tab.id, { type: 'CLEANUP_ALL' }, () => void chrome.runtime.lastError);
      const fileMap = {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      const tab = tabs[0];
      if (!tab || !tab.id) return;
      chrome.tabs.sendMessage(tab.id, message.payload || {}, () => void chrome.runtime.lastError);
    });
  }
});
